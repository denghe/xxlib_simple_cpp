...
namespace nanolog {
    ...
    struct NonGuaranteedLogger {
        explicit NonGuaranteedLogger(uint32_t ring_buffer_size_mb_) : ring_buffer_size_mb(ring_buffer_size_mb_) {}
        uint32_t ring_buffer_size_mb;
    };

    namespace {
        void format_timestamp(std::ostream &os, uint64_t timestamp) {
            auto&& t = std::chrono::system_clock::to_time_t(xx::Epoch10mToTimePoint(timestamp));
        ...
        template<typename T>
        struct is_c_string : std::integral_constant<bool,
    ...
    inline char const *to_string(LogLevel loglevel) {
        return LogLevelString[(int)loglevel];
    }
    ...
    public:
        ...
        NanoLogLine(LogLevel level, char const *file, char const *function, uint32_t line)
            ...
            encode0(xx::NowEpoch10m(), this_thread_id(), file, function, line, level);
        }
    ...
    */
    ...
    inline void initialize(NonGuaranteedLogger ngl, std::string const &log_directory, std::string const &log_file_name,
                           uint32_t log_file_roll_size_mb) {
        nanologger.reset(new NanoLogger(ngl, log_directory, log_file_name, log_file_roll_size_mb));
        atomic_nanologger.store(nanologger.get(), std::memory_order_seq_cst);
    }

    inline void initialize(GuaranteedLogger gl, std::string const &log_directory, std::string const &log_file_name,
                           uint32_t log_file_roll_size_mb) {
