...
namespace xx {
    ...
    struct UvTimer : UvItem {
        ...
        inline int Start(uint64_t const &timeoutMS, uint64_t const &repeatIntervalMS,
                         std::function<void()> &&onFire = nullptr) noexcept {
            if (!uvTimer) return -1;
    ...
    struct UvResolver : UvItem {
        ...
        inline int Resolve(std::string const &domainName, uint64_t const &timeoutMS = 3000) noexcept {
            ...
            if (int r = uv_getaddrinfo((uv_loop_t *) &uv.uvLoop, (uv_getaddrinfo_t *) &req->req,
                                       [](uv_getaddrinfo_t *req_, int status, struct addrinfo *ai) {
                                           auto req = std::unique_ptr<uv_getaddrinfo_t_ex>(
                                                   container_of(req_, uv_getaddrinfo_t_ex, req));
                                           ...
                                           do {
                                               ...
                                               if (std::find(ips.begin(), ips.end(), s) == ips.end()) {
                                                   ips.push_back(std::move(
                                                           s));                                // known issue: ios || android will receive duplicate result
                                               }
                                               ai = ai->ai_next;
                                           } while (ai);
                                           uv_freeaddrinfo(ai);
    ...
    struct UvPeerBase : UvItem {
        ...
        virtual void
        SendPrepare(BBuffer &bb, std::size_t const &reserveLen) noexcept = 0;        // resize ctx & header space to bb
        virtual int
        SendAfterPrepare(BBuffer &bb) noexcept = 0;                                // fill ctx & header & send
    ...
    struct UvDialerBase : UvItem {
        ...
        virtual int
        Dial(std::string const &ip, int const &port, uint64_t const &timeoutMS = 0, bool cleanup = true) noexcept = 0;
    ...
    struct UvPeer : UvItem {
        ...
        inline int
        SendRequest(Object_s const &msg, std::function<int(Object_s && msg)> &&cb, uint64_t const &timeoutMS) noexcept {
    ...
    struct UvTcpPeerBase : UvPeerBase {
        ...
        inline virtual int SendDirect(uint8_t *const &buf, std::size_t const &len) noexcept override {
            ...
            if (int r = uv_write(req, (uv_stream_t *) uvTcp, &req->buf, 1,
                                 [](uv_write_t *req, int status) { ::free(req); })) {
                Dispose();
                return r;
            }
            return 0;
        ...
        inline int ReadStart() noexcept {
            if (!uvTcp) return -1;
            return uv_read_start((uv_stream_t *) uvTcp, Uv::AllocCB,
        ...
        virtual int Unpack(uint8_t *const &recvBuf, uint32_t const &recvLen) noexcept {
            ...
            while (offset + 4 <= buf.len) {                            // ensure header len( 4 bytes )
                uint32_t len =
                        buf[offset + 0] + (buf[offset + 1] << 8) + (buf[offset + 2] << 16) + (buf[offset + 3] << 24);
    ...
    struct UvKcp : UvItem {
        ...
        UvKcp(Uv &uv, std::string const &ip, int const &port, bool const &isListener)
            ...
            if (int r = uv_udp_recv_start(uvUdp, Uv::AllocCB, [](uv_udp_t *handle, ssize_t nread, const uv_buf_t *buf,
        ...
        inline virtual int
        Send(uint8_t const *const &buf, ssize_t const &dataLen, sockaddr const *const &addr = nullptr) noexcept {
            if (!uvUdp) return -1;
            ...
            if (int r = uv_udp_send(req, uvUdp, &req->buf, 1, addr ? addr : (sockaddr *) &this->addr,
    ...
    struct UvKcpPeerBase : UvPeerBase {
        ...
        inline virtual int Update(int64_t const &nowMS) noexcept override {
            ...
            auto &&currentMS = uint32_t(
                    nowMS - createMS);            // known issue: uint32 limit. connect only alive 50+ days
        ...
        inline virtual int Unpack(uint8_t *const &recvBuf, uint32_t const &recvLen) noexcept {
            ...
            while (offset + 4 <= buf.len) {                            // ensure header len( 4 bytes )
                uint32_t len =
                        buf[offset + 0] + (buf[offset + 1] << 8) + (buf[offset + 2] << 16) + (buf[offset + 3] << 24);
                if (len > uv.maxPackageLength)
                    return -1;            // invalid length
                if (offset + 4 + len > buf.len) break;                // not enough data

                offset += 4;
                if (int r = peer->HandlePack(buf.buf + offset, len))
                    return r;
                offset += len;
            }
            buf.RemoveFront(offset);
    ...
    protected:
        inline virtual int
        Unpack(uint8_t *const &recvBuf, uint32_t const &recvLen, sockaddr const *const &addr) noexcept override {
            assert(port);
            ...
            if (recvLen == 4 && owner) {                        // hand shake contain 4 bytes auto inc serial
                ...
                if (idx == -1) {
                    idx = shakes.Add(ipAndPort,
                                     std::make_pair(++convId, NowSteadyEpochMS() + handShakeTimeoutMS)).index;
                }
                memcpy(recvBuf + 4, &shakes.ValueAt(idx).first,
                       4);    // return serial + conv( temp write to recvBuf is safe )
            ...
            if (peerIter == -1) {                                // conv not found: scan shakes
                ...
                memcpy(&peer->addr, addr,
                       sizeof(sockaddr_in6));// upgrade peer's tar addr( maybe accept callback need it )
    ...
    struct UvDialer : UvCreateAcceptBase {
        ...
        virtual int
        Dial(std::vector<std::string> const &ips, int const &port, uint64_t const &timeoutMS = 2000) noexcept;

        virtual int
        Dial(std::vector<std::pair<std::string, int>> const &ipports, uint64_t const &timeoutMS = 2000) noexcept;
    ...
    inline int
    UvDialerBase::Dial(std::vector<std::string> const &ips, int const &port, uint64_t const &timeoutMS) noexcept {
        if (Disposed()) return -1;
        Cancel();
        for (auto &&ip : ips) {
    ...
    inline int
    UvDialerBase::Dial(std::vector<std::pair<std::string, int>> const &ipports, uint64_t const &timeoutMS) noexcept {
        if (Disposed()) return -1;
        Cancel();
    ...
    struct UvKcpDialer : UvDialerBase {
        ...
        inline virtual int Dial(std::string const &ip, int const &port, uint64_t const &timeoutMS = 0,
                                bool cleanup = true) noexcept override {
            if (disposed) return -1;
            if (cleanup) {
                Cancel();
            }
    ...
    inline UvDialer::UvDialer(Uv &uv, int const &tcpKcpOpt)
            : UvCreateAcceptBase(uv), onConnect(this->onAccept) {
    ...
    inline int
    UvDialer::Dial(std::vector<std::string> const &ips, int const &port, uint64_t const &timeoutMS) noexcept {
        if (int r = SetTimeout(timeoutMS)) return r;
        if (tcpDialer) {
    ...
    inline int
    UvDialer::Dial(std::vector<std::pair<std::string, int>> const &ipports, uint64_t const &timeoutMS) noexcept {
        if (int r = SetTimeout(timeoutMS)) return r;
        if (tcpDialer) {
