...
namespace xx::Epoll {
    ...
    struct TcpPeer : Timer {
        ...
        int Send(Data &&data);
        // 会导致 关闭 fd 解除映射, fd = -1. 如果 reason 传 0 则不触发 OnDisconnect
        void Close(int const& reason) override;
        // 判断 peer 是否还活着( 没断 )
    ...
    inline void Item::Close(int const& reason) {
        if (fd != -1) {
    ...
    inline void TcpPeer::Close(int const& reason) {
        if (fd == -1) return;
        if (reason) {
            OnDisconnect(reason);
        }
        // remove from epoll control, close fd, fd = -1
        this->Item::Close(0);
    }
